...
-- Redacted feature, as it wasn't that useful: AltDim set to 1 causes each region of same colour to slightly alternate in brightness. 0 disables this.
AltDim = 0
...
for i = 0, reaper.CountProjectMarkers()-1 do
  ...
  if ((arg1<pos and pos<arg2) or (pos<arg1 and rgnend>arg1)) and (name~="")and (isrgn==true) then
  -- the above makes sure the region is in time selection
      -- needs additional check for if term already exists in list.
      if findMatchIndex(uniqueMarkerNames, name) then
      else
